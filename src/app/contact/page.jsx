  ...
  return (
    ...
    >
      <div className="h-full flex flex-col lg:flex-row px-4 sm:px-8 md:px-12 lg:px-20 xl:px-48">
        {/*TEXT CONTAINER*/}
        {!isKeyboardOpen && (
          <div className="h-1/6 lg:h-full lg:w-1/2 flex items-center justify-center text-4xl lg:text-6xl">
        ...
        <motion.form
          onSubmit={sendEmail}
          ref={form}
          className="h-5/6 lg:h-full lg:w-1/2 ring-2 ring-[#271624] rounded-xl text-xl flex flex-col gap-4 lg:gap-8 justify-center p-4 md:p-12 lg:p-24"
          initial={{ opacity: 0, y: "100vh" }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 1, ease: "easeOut" }}
        >
          {!isKeyboardOpen && <span className="font-Synonym">Hi Carl,</span>}
          <textarea
            rows={3}
            className="bg-transparent border-b-2 border-b-[#271624] outline-none resize-none focus:bg-[#EFEFDC]  focus:z-10"
            name="user_message"
            required
          />
          ...
          <input
            ...
            required
          />
          {!isKeyboardOpen && <span className="font-Synonym">Regards</span>}
          {!isKeyboardOpen && (
            <button
